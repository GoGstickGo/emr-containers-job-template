...
func loadConfigFromEnv(logger *logrus.Logger) (Config, error) {
	cfg := Config{
		AWSRegion: getEnv("AWS_REGION", "us-east-1"),
		PathYAML:  getEnv("PATH_YAML", "example.yaml"),
		SSMName:   os.Getenv("SSM_NAME"), // Must be set; no default
	}
...
func processJobTemplate(ctx context.Context, logger *logrus.Logger, clients *awsutils.AWSClients, jobTemplate template.JobTemplateConfig, cfg Config, random rand.Rand) error {
	...
	if err = awsutils.UpdateSSMParameter(ctx, clients.SSM, cfg.SSMName, jobTemplateID); err != nil {
		return fmt.Errorf("failed to update SSM parameter '%s': %w", cfg.SSMName, err)
	}
	logger.Infof("Updated SSM parameter '%s' with job template ID: %s", cfg.SSMName, jobTemplateID)
